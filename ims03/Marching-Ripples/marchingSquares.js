  ...
  addLines(isovalues, colors, addFade, alpha) {
    ...
    for (let i = 0; i < this.cols; i++) {
      ...
      for (let j = 0; j < this.rows; j++) {
        ...
        this.field[idx] = this.useOSN ? float(this.noise.noise3D(xoff, yoff, this.zoff)) : noise(xoff, yoff);
        yoff += this.increment;
      }
      xoff += this.increment;
    }
    ...
    for (let level = 0; level < isovalues.length; level++) {
      ...
      for (let i = 0; i < this.cols - 1; i++) {
        for (let j = 0; j < this.rows - 1; j++) {
          ...
          const state = this.getState(a >= iso ? 1 : 0, b >= iso ? 1 : 0, c >= iso ? 1 : 0, d >= iso ? 1 : 0);
